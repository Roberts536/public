#!/bin/bash
: '
This script is almost unchanged from that for krypton4.
I just used it a few times with different key lengths.
On one attempt the key was almost decoded, so I guessed it :)
'

# Break a Vignere cipher of given length
lowalph=({a..z})
upalph=({A..Z})
key_length=$2
declare -a key
ciphertext=$(cat $1 | sed s/' '//g)
pwd_enc=$(cat ./krypton5 | sed s/' '//g)
